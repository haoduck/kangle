...
phpv=`"$PREFIX"/bin/php -v |grep "$PHP72" -o`
file="php-$PHP72.tar.bz2"
ZEND_ARCH="i386"
LIB="lib"
force_install=$1
...
if [ "$PHP72" = "$phpv" ]&&[ "$force_install" != "force"  ];
then {
clear
echo -e "————————————————————————————————————————————————————
检测到您已安装了 PHP-"$phpv" 
...
if [ ! -s "$file" ];
then {
wget $DOWNLOAD_FILE_URL/file/php-$PHP72.tar.bz2 -O php-$PHP72.tar.bz2
tar xjf php-$PHP72.tar.bz2
}
else {
...
echo "正在下载PHP7配置文件"
wget -q $DOWNLOAD_URL/config_file/php72.xml -O $PREFIX/config.xml
wget -q $DOWNLOAD_URL/config_file/php72.ini -O $PREFIX/php-templete.ini
#install ioncube
echo "正在下载ioncube组件"
wget -q $DOWNLOAD_FILE_URL/file/ioncube-$ZEND_ARCH-7.2.zip
echo "正在解压文件"
unzip ioncube-$ZEND_ARCH-7.2.zip
echo "正在安装ioncube组件"
mkdir -p $PREFIX/ioncube
rm -rf $PREFIX/ioncube/ioncube_loader_lin_7.2.so
mv -f ioncube_loader_lin_7.2.so $PREFIX/ioncube/ioncube_loader_lin_7.2.so
echo "ioncube组件安装完成"
#install SourceGuardian
wget $DOWNLOAD_FILE_URL/file/ixed-$ZEND_ARCH-7.2.zip -O ixed-$ZEND_ARCH-7.2.zip
unzip ixed-$ZEND_ARCH-7.2.zip
mkdir -p $PREFIX/ixed
mv -f ixed.7.2.lin $PREFIX/ixed/ixed.7.2.lin
